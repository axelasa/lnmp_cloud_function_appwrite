...
MpesaCredentials credentialsFromEnv(final context) {
  ...
  try {
    ...
    switch (method) {
      ...
      case HttpMethods.get:
        ...
        final MpesaCredentials credentials = credentialsFromEnv(context);
        ...
        await sentStkPush(
          ...
          onSuccess: (map) {
            res.send('Stk push initiated', 200, defaultHeaders);
          },
        );
      ...
      case HttpMethods.post:
        // final String? project = Platform
        //     .environment['APPWRITE_FUNCTION_PROJECT_ID'];
        // final String? key = Platform.environment['APPWRITE_API_KEY'];
        // final String endPoint = Platform.environment['END_POINT']!;
        // context.log('END_POINT => $endPoint ');
        // context.log('CLIENT_PROJECT => $project');
        // context.log('CLIENT_KEY => $key');
       ...
       /*   final document = await databases.createDocument(
              ...
              documentId: callback.checkoutRequestID,
              data: callback.toJson(),
          );


          context.log("Document created at ${document?.$createdAt} @ ${document?.$databaseId} --> ${document?.$collectionId} --> @ ${document?.$id}");
  */
